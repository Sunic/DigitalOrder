...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if(!moveChanged && fabs(location.x - lastLoction.x) > 60 && fabs(location.y - lastLoction.y) < 25){
        ...
        //手势添加，翻页效果
		[UIView setAnimationTransition:( location.x - lastLoction.x > 0? UIViewAnimationTransitionCurlDown : UIViewAnimationTransitionCurlUp) forView:self.view cache:YES];
